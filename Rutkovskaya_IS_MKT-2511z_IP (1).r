...
# Экспортируем основной датасет с меткой тональности
write.csv(df, "dashboard_data.csv", row.names = FALSE)

# Экспортируем таблицу топ-слов для визуализации лексики
write.csv(top_words, "top_words.csv", row.names = FALSE)

# Экспортируем AFINN-скоры на уровне отзыва
write.csv(review_afinn, "afinn_scores.csv", row.names = FALSE)
